...
| [#171](https://github.com/aws-solutions-library-samples/guidance-for-aws-deepracer-event-management/pull/171) → [#203](https://github.com/aws-solutions-library-samples/guidance-for-aws-deepracer-event-management/pull/203) | feat: racer avatar, highlight colour, and identity display (RacerProfile DDB table) | — | None | ✅ **#171 merged 2026-05-04** (`14ba923`) with the original Cognito-attr design. Architecture reworked afterward because Steve flagged the staleness problem with the snapshot model and Cognito's per-pool attribute limits — Cognito custom attrs replaced by a dedicated DynamoDB `RacerProfile` table with live-join via AppSync field resolvers on `LeaderBoardEntry.profile` and `Overlay.profile`. Rework opened as **#203** on 2026-05-08 (`bf6026f`). Spec/plan: `docs/superpowers/specs/2026-05-05-racer-profile-table-design.md` and `docs/superpowers/plans/2026-05-05-racer-profile-table.md`. Two post-deploy fixes incorporated: `4ca6e4f` (main website graphql codegen had stale avatar/highlight selections — added `profile { ... }` blocks across `getLeaderboard`, `onNewLeaderboardEntry`, `onUpdateLeaderboardEntry`, `onNewOverlayInfo`, dropped stale `$avatarConfig` arg from `updateOverlayInfo`) and `bf6026f` (Lambda's `addLeaderboardEntry`/`updateLeaderboardEntry` mutations needed `profile { ... }` in their return selection so the field resolver fires on subscription deliveries — without it, avatars vanished after race events for connected WebSocket subscribers). Pipeline deployed green to fork dev; smoke-tested. Steve's UX work (silhouette `AvatarDisplay`, neutral default avatar, mini avatar in top nav, preview in collapsed header) preserved end-to-end. Standalone overlays app (`website/overlays/`) avatar render deferred to task #29 (overlay redesign moves to HTML+CSS). |
