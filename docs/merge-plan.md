...
The remaining five still-open Tier 1 PRs are listed below — all rebased onto post-#170
main, mergeable, no schema/CFN-resource changes:

| PR | Title | Closes | Surface |
|---|---|---|---|
| [#176](https://github.com/aws-solutions-library-samples/guidance-for-aws-deepracer-event-management/pull/176) | fix(leaderboard): scroll to and highlight racer when race submitted | [#40](https://github.com/aws-solutions-library-samples/guidance-for-aws-deepracer-event-management/issues/40) | Leaderboard frontend only — Steve flagged the new-finisher overlay obscuring the bottom entry; fixed in `805c96c` + `b97e9db` (entries height shrinks while overlay visible, no transition so scrollIntoView geometry stays correct), back-to-top timer rescue (`2195e02`, 60s), highlight overflow past leaderboard borders (`8a92177`). |
